...
- **`<InlineImage>` `resource` must name a drawable that actually exists.** The Watch Face Studio export idiom is `<InlineImage resource="..." source="COMPLICATION.MONOCHROMATIC_IMAGE" ...>`, where `source` supplies the real icon at runtime and `resource` is only a design-time fallback. Older exports used `resource="empty"` — a dangling reference (there is no `empty` drawable), which Wear OS 6 tolerated but Wear OS 7 does not: the element fails to inflate and complication icons disappear. All 56 sites now point at the real `index_icon_2` drawable. Never point `resource` at a name with no file in `res/drawable-nodpi/`.
- **Known IDE false-positive error** in `watchface.xml`: `<expr> expected` on `<Parameter expression="&#160;" />`. This is a Watch Face Studio export idiom — `&#160;` injects literal whitespace. It doesn't affect the build; don't "fix" it without testing the rendered watch face.
